...
| `SNOWFLAKE_PAT` | **Required — key-pair is dead.** `N8N_OPS` is `TYPE=PERSON`, so the account MFA authentication policy rejects key-pair before it ever checks the key ("Authentication attempt rejected by the current authentication policy"); PAT is an allowed method for person-type users. Set it and it wins over the key-pair vars, which stay in place as the rollback. Expires **2027-09-09**. ⚠️ Same failure mode as `UC_*`: it rides a PERSONAL account (`sankeerth.r@`) and dies on offboarding. The real fix is a `TYPE=SERVICE` user with the key registered — then drop this var and key-pair resumes with no code change. |
